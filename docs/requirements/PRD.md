...
| R-013 | 在 GNOME/X11 环境下状态栏菜单可见，核心管理功能（开关、热键配置、LLM 表单、模型切换/下载）与 macOS 等效 | Linux 环境手工验收（多桌面环境） |
...
- 待确认问题：
  ...
  - Linux 状态栏菜单已列入正式需求(R-013)，具体实现方案待定（AppIndicator/System Tray）
  - R-013 已将 Linux 状态栏菜单列入正式需求
