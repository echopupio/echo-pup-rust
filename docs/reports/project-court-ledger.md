...
- 时间：2026-04-17 12:18:00 +0800
- 结论：已确认 Paraformer 下载链路存在文件落盘语义缺陷。原实现直接写最终文件名，下载失败时会残留 0B 或半截文件，后续重试又可能把“存在且非空”的残留文件误判为已完成，导致状态栏反复下载失败或留下不可用模型。
- 备注：
  - 已将单文件下载改为先写 `*.part` 临时文件，完成后再原子重命名。
  - 已补充已有文件与远端 `Content-Length` 对比，不再对尺寸不匹配文件直接跳过。
  - 已补充空文件清理与更具体的路径级错误信息，便于后续复盘真实失败点。

### ASR 解码修复

- 时间：2026-04-17
- 结论：sherpa-onnx OnlineRecognizer 的 `decode_audio()` 实现缺少 `input_finished()` 和循环 decode，导致音频帧几乎未被处理，ASR 始终返回空结果。已修复。
- 涉及文件：`src/asr/sherpa_paraformer.rs`

### 流式草稿提交功能

- 时间：2026-04-17
- 结论：已实现"边说边出文字"功能。录音期间 ASR 中间结果实时输入到光标处，录音结束时替换为最终文本。
- 备注：
  - 通过退格键删除旧草稿 + 输入新草稿实现，兼容大多数编辑器。
  - 默认开启，可通过菜单或 config.toml `[commit] streaming_draft` 关闭。
  - 菜单开关变更在下次录音时生效，不影响进行中的录音。
  - 富文本编辑器（Notion/Figma）退格行为可能不标准，建议用户在此类场景关闭。
  - 新增 4 个单测，全部 59 测试通过。
