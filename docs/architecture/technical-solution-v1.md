...
当前重点改造方向：
- 提取共享菜单业务内核，供 TUI 与状态栏共同调用。
- 将状态栏通信升级为双向 IPC，状态栏只负责展示/交互，主进程负责执行。
...
- 子进程通信契约：
  - 现状：状态栏通过标准输入接收状态行协议。
  - 目标：升级为双向 IPC（建议 NDJSON）承载菜单动作请求、执行结果、状态快照与下载进度。

## 5. 上线计划

- 里程碑 1：共享菜单内核重构（TUI 接入共享逻辑）。
- 里程碑 2：状态栏双向 IPC 与菜单动作打通。
- 里程碑 3：下载进度可视化与完整回归验收。
